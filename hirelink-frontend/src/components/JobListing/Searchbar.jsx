...
function Searchbar({ setSearch, setSelectedLocation }) {
  ...
  return (
    <>
      <div className="flex flex-col text-text-secondary items-center py-5 md:py-2 px-6 md:px-5 border shadow rounded-xl gap-6 md:gap-10 font-light lg:flex-row">
        {/* first */}
        <div className="flex items-center  w-full gap-3 lg:border-r-2 border-neutral-300  lg:w-[45%] ">
          <div>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-5 justify-between w-full lg:w-[55%] ">
          <div className="flex justify-start lg:justify-center items-center gap-3 w-full">
            ...
            <div className="w-full relative">
              ...
              />
              {location.length > 0 && (
                <div className="absolute left-0 mt-2 w-52 rounded-md shadow-lg bg-background z-10">
                  <ul className="py-1 text-base leading-6 rounded-md ring-1 ring-neutral-800 ring-opacity-5 overflow-auto max-h-60">
                    {location.map((item, index) => {
                      return (
                        <li
                          key={index}
                          className="text-text-primary select-none relative py-2 pl-3 pr-9 hover:bg-neutral-200 cursor-pointer"
